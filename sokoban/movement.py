...
class Movement():
    ...
    def move_up(self):
        for node in self.unoccupied_nodes:
            if self.future_hitbox.get('up').colliderect(node.hitbox):
                self.y -= 60
                return True
        return False
    ...
    def move_down(self):
        for node in self.unoccupied_nodes:
            if self.future_hitbox.get('down').colliderect(node.hitbox):
                self.y += 60
                return True
        return False
    ...
    def move_right(self):
        for node in self.unoccupied_nodes:
            if self.future_hitbox.get('right').colliderect(node.hitbox):
                self.x += 60
                return True
        return False
    ...
    def move_left(self):
        for node in self.unoccupied_nodes:
            if self.future_hitbox.get('left').colliderect(node.hitbox):
                self.x -= 60
                return True
        return False